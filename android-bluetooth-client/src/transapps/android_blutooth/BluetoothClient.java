...
public class BluetoothClient extends Activity {
    ...
    public static String TAG = "BluetoothClient";
...
//					}
	...
	protected void onCreate(Bundle savedInstanceState) {
			...
			public void handleMessage(Message msg) {
				...
				case MESSAGE_READ:
					...
					if (readMessage.length() > 0) {
                        // read the message and do things with it.  I tend to package this into a bundle with
                        // key value pairs that i can pull from but do whatever you want..
					}
					break;
				default:
					break;
		...
		setContentView(R.layout.bluetooth_activity_layout);
//		if(navTool == null) {
//			navTool = new NavTool(this);
//			navTool.publishNavCard();
//		}
...
//		}
	...
	public void onDestroy() {
		...
		//			tts.shutdown();
		//		if(navTool != null) {
		//			navTool.unpublishCard(this);
		//		}
		//		curAlert.unpublishCard();
		super.onDestroy();
	...
	private void detectAndSetUp() {
		...
		if (pairedDevices.size() > 0) {
			...
			for (BluetoothDevice device : pairedDevices) {
				...
				//} else {
				if(device.getName()!=null && ! device.getName().contains("Phone Station")){
					btArray.add(device.getName() + "\n" + device.getAddress());
				}
//				}
				// Add the name and address to an array adapter to show in a ListView
	...
	private class ConnectThread extends Thread {
		...
		public ConnectThread(BluetoothDevice device) {
			...
			try {
				...
				// uuids are the accepted UUIDS... is the app's UUID string, also used by the server code
				//				for(int i =0; i< uuids.length;i++) {

				// works on my device...
				//tmp = device.createRfcommSocketToServiceRecord(uuids[0]);	

				// maybe works on the note3?
